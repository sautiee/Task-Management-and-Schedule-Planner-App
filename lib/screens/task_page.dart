...
import 'package:taskmanagement/widgets/animated_coin_popup.dart';
import 'package:taskmanagement/widgets/delete_chip.dart';
import 'package:taskmanagement/widgets/dialog_box.dart';
import 'package:taskmanagement/widgets/task_info.dart';
import 'package:taskmanagement/widgets/task_tile.dart';
...
class _TaskPageState extends State<TaskPage> {
  ...
  void checkBoxChanged(bool? value, int index) async {
    ...
    if (isChecked && !db.taskList[index][1] && !rewardGiven) {
      ...
      if (lastDate == null || todayDate.difference(DateTime(lastDate.year, lastDate.month, lastDate.day)).inDays != 0) {
        ...
        int longestStreak = _box3.get("LONGEST_STREAK", defaultValue: 0);
        if (streak > longestStreak) {
          _box3.put("LONGEST_STREAK", streak);
        }
  ...
  String selectedCategory = "Main"; // default value
...
///------------------------------ ANIMATIONS ------------------------------///
  ...
  bool _showCoinPopup = false;
  late OverlayEntry _coinPopupEntry;

  void showCoinPopup(int amount) {
    final popupKey = GlobalKey<AnimatedCoinPopupState>();
    late OverlayEntry entry;
    entry = OverlayEntry(
      builder: (context) => Positioned(
        top: 70,
        right: 15,
        child: AnimatedCoinPopup(amount: amount, key: popupKey),
      ),
    );
    ...
    Future.delayed(const Duration(milliseconds: 1200), () async {
      if (popupKey.currentState != null) {
        await popupKey.currentState!.fadeOut();
      }
      entry.remove();
    });
  }

// Filters or FilterChips to show the categories
Widget categoryChips() {
  return Padding(
    ...
    child: GestureDetector(
      ...
      child: Wrap(
        spacing: 9,
        children: db.category.map((category) {
...
Widget build(BuildContext context) {
  ...
  final filteredTasks = db.taskList.where((task) => task[6] == selectedCategory).toList();
  final uncompletedTasks = filteredTasks.where((task) => task[1] == false).toList();
  final completedTasks = filteredTasks.where((task) => task[1] == true).toList();
  ...
  return Scaffold(
    ...
    child: GestureDetector(
      ...
      child: FloatingActionButton(
        ...
        child: Icon(_isListening ? Icons.mic : Icons.add, size: 28,),
      ),
    ),
  ),
    ...
    body: SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          //uncompleted tasks
          ...uncompletedTasks.asMap().entries.map((entry) {
              ...
              child: GestureDetector(
                child: AnimatedSwitcher(
                  ...
                  child: TaskTile(
                    ...
                    priority: db.taskList[index][3],
                    date: db.taskList[index][2],
                    time: db.taskList[index][5],
                    taskURL: db.taskList[index].length > 7 &&
                            db.taskList[index][7] is String &&
                            (db.taskList[index][7] as String).trim().startsWith("https://")
                        ? (db.taskList[index][7] as String).trim()
                        : null,
          ...
          //completed tasks
          AnimatedOpacity(
            opacity: _showCompletedTasks ? 1.0 : 0.0,
            duration: Duration(milliseconds: 100),
            child: IgnorePointer(
              ignoring: !_showCompletedTasks,
              child: Column(
                children: completedTasks.asMap().entries.map((entry) {
                  int index = db.taskList.indexOf(entry.value);
                  var task = entry.value;
                  String priorityLevel = db.taskList[index][3];
                  return AnimatedSwitcher(
                    ...
                    child: TaskTile(
                      ...
                      time: db.taskList[index][5],
                      taskURL: db.taskList[index].length > 7 ? db.taskList[index][7] as String? : null,
                    ),
                  );
                }).toList(),
              ),
